...
const form = ref({ nguoi_nhan: '', ly_do: '', so_tien: 0, hinh_thuc: 'tien_mat' });
...
async function fetchData() {
  ...
  } catch (err) { handleApiError(err, toast, 'Không thể tải danh sách phiếu chi'); }
  loading.value = false;
}

function openNew() {
  form.value = { nguoi_nhan: '', ly_do: '', so_tien: 0, hinh_thuc: 'tien_mat' };
  editing.value = false;
...
async function save() {
  try {
    if (editing.value) {
      await api.put(`/phieu-chi/${form.value.id}`, form.value);
      toast.add({ severity: 'success', summary: 'Đã cập nhật', life: 2000 });
    } else {
      await api.post('/phieu-chi', form.value);
      toast.add({ severity: 'success', summary: 'Đã tạo phiếu chi', life: 2000 });
    }
...
async function huy(row) {
  ...
  try {
    await api.patch(`/phieu-chi/${row.id}/huy`);
    toast.add({ severity: 'warn', summary: 'Đã hủy', life: 2000 });
    fetchData();
  } catch (err) { handleApiError(err, toast, 'Không thể hủy phiếu chi'); }
}

function openPDF(id) {
  api.get(`/phieu-chi/${id}/pdf-preview`).then(res => {
...
<template>
  <div class="animate-fade-in">
    <PageHeader title="Phiếu chi" icon="pi pi-credit-card">
      <template #actions>
    ...
    <div class="card">
      <DataTable :value="list" :loading="loading" :totalRecords="totalRecords" :rows="limit" :lazy="true"
        ...
        <Column header="Ngày" style="width: 80px;">
          <template #body="{ data }">{{ fmtDate(data.ngay_chi) }}</template>
        </Column>
        ...
        <Column header="Số tiền" style="width: 100px; text-align: right;">
          <template #body="{ data }"><span style="font-weight: 600; color: #dc2626;">{{ fmt(data.so_tien) }}đ</span></template>
        </Column>
    ...
    <Dialog v-model:visible="dialogVisible" :header="editing ? 'Sửa phiếu chi' : 'Lập phiếu chi'" :modal="true" :style="{ width: '420px' }" class="compact-dialog">
      <div class="form-grid-1">
        <div class="form-group">
          <label class="form-label">Người nhận</label>
          <InputText v-model="form.nguoi_nhan" placeholder="Tên người nhận tiền" fluid />
        </div>
        <div class="form-group">
          <label class="form-label">Lý do</label>
          <InputText v-model="form.ly_do" placeholder="Nội dung chi" fluid />
        </div>
        <div class="form-group">
